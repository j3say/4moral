...
class _SearchBottomSheetState extends State<SearchBottomSheet> {
  ...
  String activeTab = 'All'; // Default tab
      ...
      builder: (BuildContext context, ScrollController scrollController) {
        return NotificationListener<DraggableScrollableNotification>(
          onNotification: (notification) {
            bool nowExpanded =
                notification.extent > widget.initialChildSize + 0.1;
            if (_isExpanded != nowExpanded) {
              _isExpanded = nowExpanded;
              if (widget.onSearchExpanded != null) {
                widget.onSearchExpanded!(_isExpanded);
              }
            }
            return true;
          },
                          ...
                          searchc.text != ""
                              ? activeTab == 'All'
                                  ? (filteredUserList.isNotEmpty ||
                                      ...
                                      ? _buildAllResultsList(scrollController)
                                      : _buildNoResults()
                                  : activeTab == 'Users'
                                  ? filteredUserList.isNotEmpty
                                  ...
                                  ? Obx(
                                    () =>
                                        exploreCnt
                                                .filteredPostDataList
                                                .isNotEmpty
                                            ? _buildPostList(scrollController)
                                            : _buildNoResults(),
                                  )
                                  : activeTab == 'Products'
                                  ? filteredProductList.isNotEmpty
                                      ...
                                      : _buildNoResults()
                                  : filteredHashtagsList.isNotEmpty
                                  ? _buildHashtagList(scrollController)
                                  : _buildNoResults()
                              : _buildInitialState(),
                    ),
                  ],
  ...
  Widget _buildAllResultsList(ScrollController scrollController) {
    return ListView(
  ...
  Widget _buildPostList(ScrollController scrollController) {
    return Obx(
      () => ListView.builder(
        controller: scrollController,
        physics: const BouncingScrollPhysics(),
  ...
  Widget _buildHashtagList(ScrollController scrollController) {
    return ListView.builder(
      controller: scrollController,
      physics: const BouncingScrollPhysics(),
  ...
  Widget _buildInitialState() {
    return Stack(
      ...
      children: [
        ...
        SizedBox(height: 220, child: Lottie.asset("assets/search.json")),
      ],
    );
  }
}
